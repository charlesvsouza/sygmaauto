...
- **Camada 5 (Financeiro/Admin/Público — FINAL):** migradas Financial, DRE, KPIs, Commissions, Reports, NPS, NpsAnswer, SuperAdmin, SuperAdminLogin, Settings, Maintenance, Users, Dashboard, DashboardRetifica, Whatsapp, ActivateTenant, ProvisionTenantModal, PlanFeatureRoute, RetificaModeRoute, e as públicas Blog/Manual/Support. `alert()` removido de Financial, DRE, Commissions, Reports (2), Settings, Users → `useToast()`. **As classes compartilhadas do `index.css` (`.card`, `.btn*`, `.input`, `.badge*`, `.table`, `.modal*`, `.stat*`, `.empty-state*`, `.avatar`, `.tabs`, `.dropdown`, etc.) foram finalmente convertidas para dark premium** — seguro agora que nenhuma tela clara resta.
- **STATUS: migração concluída.** Zero `slate/zinc` estruturais em `src/`. `npm run build` passa. Identidade dark premium + gold aplicada em todo o app.
