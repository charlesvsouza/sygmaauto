...
import { OrbitalHero } from '../components/marketing/OrbitalHero';
...
export function LandingPage() {
  ...
  return (
    <MarketingShell>
      {/* Hero */}
      <section className="relative max-w-6xl mx-auto px-4 sm:px-6 pt-10 sm:pt-16 pb-20 sm:pb-28">
        <div className="grid lg:grid-cols-[1.2fr_0.8fr] gap-10 items-center">
          <div>
            ...
            <motion.h1
              ...
              className="mt-6 text-[clamp(2.1rem,5.6vw,3.8rem)] font-black leading-[0.95] tracking-tight"
            >
            ...
            <motion.div
              ...
              className="mt-5"
            >
              <EcgPulse className="h-7 w-full max-w-[320px] overflow-hidden relative" />
            </motion.div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {features.map(({ icon: Icon, title, desc }, index) => (
            <motion.article
              ...
              className="rounded-xl border border-line bg-ink/5 backdrop-blur-sm p-6 hover:border-[#ff7b2f]/35 transition-colors"
            >
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 py-16 sm:py-20">
        ...
        <div className="grid lg:grid-cols-2 gap-x-12 gap-y-10">
          <div>
            <p className="text-[11px] uppercase tracking-[0.2em] text-emerald-500 font-bold">Inovações disponíveis</p>
            <h3 className="mt-2 text-xl font-black text-white">Já em uso por oficinas</h3>
            <div className="mt-6">
              {releasedInnovations.map((item, index) => (
                <motion.div
                  key={item.title}
                  initial={{ opacity: 0, y: 16 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  ...
                  className="flex gap-4 py-5 border-b border-line last:border-b-0"
                >
                  <span className="text-2xl font-black text-white/25 tabular-nums">{String(index + 1).padStart(2, '0')}</span>
                  <div>
                    <div className="flex items-center gap-2">
                      <p className="text-[11px] uppercase tracking-[0.12em] text-emerald-500/90 font-bold">{item.tag}</p>
                      <span className="text-[11px] text-white/40">{item.date}</span>
                    </div>
                    <p className="mt-1.5 text-sm font-bold text-white">{item.title}</p>
                    <p className="mt-1.5 text-xs text-white/65 leading-relaxed">{item.excerpt}</p>
                  </div>
                </motion.div>
              ))}
            </div>
          ...
          <div>
            ...
            <h3 className="mt-2 text-xl font-black text-white">Próximos passos do roadmap</h3>
            <div className="mt-6">
        ...
        <div className="mt-10 rounded-lg border border-[#58c3cb]/25 bg-[#58c3cb]/8 px-5 py-4 text-sm text-[#d8edf0]">
      ...
      <section id="planos" className="max-w-6xl mx-auto px-4 sm:px-6 py-16 sm:py-20">
        ...
        <div className="mt-14 rounded-xl border border-[#ff7b2f]/20 bg-[#ff7b2f]/6 p-6 md:p-8">
              ...
              <tbody>
                {planCapabilities.map((row) => {
                  const renderCell = (value: string) => {
                    ...
                    return (
                      <span className={`inline-flex items-center gap-1.5 ${enabled ? 'text-emerald-500' : 'text-white/60'}`}>
