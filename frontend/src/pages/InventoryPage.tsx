  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        <ChevronDown className={cn('w-4 h-4 text-surface-500 transition-transform', open && 'rotate-180')} />
      </button>
        ...
        {open && (
          ...
          >
            ...
            <div className="max-h-48 overflow-y-auto">
              ...
              {filtered.length === 0 ? (
                <p className="px-4 py-3 text-sm text-surface-500 text-center">Nenhum fornecedor encontrado</p>
              ) : (
                filtered.map((s) => (
                  <button
                    key={s.id}
...
export function InventoryPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!canManageParts) {
      toast.error('Voce nao tem permissao para cadastrar ou editar pecas.');
      return;
    }
    try {
  ...
  const handleMovement = async () => {
    ...
    if (!canManageParts) {
      toast.error('Voce nao tem permissao para movimentar estoque. Use MASTER ou ADMIN.');
      return;
    }
    try {
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6 pb-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          ...
          { label: 'Abaixo do Mínimo', value: lowStock, icon: AlertTriangle, color: lowStock > 0 ? 'bg-red-500' : 'bg-surface-500' },
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }}
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-5 overflow-y-auto">
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    <label className="text-xs font-bold text-surface-400 uppercase tracking-wider">Fornecedor</label>
