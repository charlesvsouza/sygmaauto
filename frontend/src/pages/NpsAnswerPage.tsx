...
export function NpsAnswerPage() {
  ...
  if (!formData) return (
    <div className="min-h-screen flex items-center justify-center bg-surface-950/40">
      ...
      <div className="w-full max-w-lg bg-surface-900 rounded-lg shadow-lg p-8 space-y-8">
        ...
        <div className="text-center">
          ...
          {formData.customer?.name && (
            <p className="text-surface-400 mt-1">Olá, <strong>{formData.customer.name}</strong>!</p>
          )}
        ...
        {/* Score */}
        <div>
          <p className="text-sm font-medium text-surface-200 mb-3 text-center">
            ...
            {SCORES.map((n) => (
              <button type="button"
                ...
                className={`w-10 h-10 rounded-full text-sm font-bold border-2 transition-all
                  ${score === n ? scoreColor(n) : `bg-surface-900 ${scoreIdleColor(n)}`}`}
              >
                {n}
              </button>
            ))}
