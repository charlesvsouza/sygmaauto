  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-6 bg-black/70 backdrop-blur-sm">
      <motion.div initial={{ opacity: 0, y: 12, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }}
        className="w-full max-w-2xl bg-surface-900 border border-white/10 rounded-[2rem] shadow-2xl overflow-hidden">
        <div className="px-8 py-6 border-b border-white/10 flex items-center justify-between">
          <div>
            <h2 className="text-xl font-black text-white">Provisionar Novo Tenant</h2>
            <p className="text-sm text-surface-400">Cria um tenant pendente e gera o link de ativação do primeiro MASTER.</p>
          </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid md:grid-cols-2 gap-4 text-sm">
                <div className="bg-white/5 rounded-2xl p-4">
                  <p className="text-surface-400 mb-1">Email de ativação</p>
                  <p className="text-white font-semibold break-all">{result.inviteEmail}</p>
                </div>
                <div className="bg-white/5 rounded-2xl p-4">
                  <p className="text-surface-400 mb-1">Expira em</p>
                  <p className="text-white font-semibold">{new Date(result.expiresAt).toLocaleString('pt-BR')}</p>
                </div>
              </div>

              <div className="bg-surface-950/60 border border-white/10 rounded-2xl p-4">
                <p className="text-surface-400 text-sm mb-2">Link de ativação</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="grid md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-xs font-semibold text-surface-200 uppercase tracking-widest">Nome do Tenant</label>
                  <div className="relative">
                    <Building className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-surface-400" />
                    <input value={tenantName} onChange={(e) => setTenantName(e.target.value)} required className="w-full bg-surface-950/60 border border-white/10 rounded-xl py-3 pl-10 pr-4 text-white text-sm outline-none focus:ring-2 focus:ring-blue-500/40" placeholder="Oficina Exemplo" />
                  </div>
                </div>
                <div className="space-y-2">
                  <label className="text-xs font-semibold text-surface-200 uppercase tracking-widest">Email de Ativação</label>
                  <div className="relative">
                    <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-surface-400" />
                    <input type="email" value={inviteEmail} onChange={(e) => setInviteEmail(e.target.value)} required className="w-full bg-surface-950/60 border border-white/10 rounded-xl py-3 pl-10 pr-4 text-white text-sm outline-none focus:ring-2 focus:ring-blue-500/40" placeholder="cliente@email.com" />
                  </div>
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-xs font-semibold text-surface-200 uppercase tracking-widest">Plano</label>
                  <div className="relative">
                    <Package2 className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-surface-400" />
                    <select value={planName} onChange={(e) => setPlanName(e.target.value)} disabled={loadingPlans} className="w-full bg-surface-950/60 border border-white/10 rounded-xl py-3 pl-10 pr-4 text-white text-sm outline-none focus:ring-2 focus:ring-blue-500/40">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-semibold text-surface-200 uppercase tracking-widest">Documento Inicial</label>
                  <input value={document} onChange={(e) => setDocument(e.target.value)} className="w-full bg-surface-950/60 border border-white/10 rounded-xl py-3 px-4 text-white text-sm outline-none focus:ring-2 focus:ring-blue-500/40" placeholder="Opcional" />
                </div>
              </div>
